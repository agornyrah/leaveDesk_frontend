...
const emailRules = [s => {
  ...
  if (!/.+@.+\..+/.test(s)) return 'Email must be valid'
  return true
}]
...
async function onSubmit() {
  ...
  try {
    const success = await employeeStore.createEmployee(name.value, email.value, role.value, department.value, password.value)
    if (success) {
      emit('saved')
      closeDialog()
    } else {
      errorMessage.value = employeeStore.error || 'Failed to create employee. Please try again.'
    }
  } catch (err) {
    ...
    <v-card class="pa-6 rounded-xl" elevation="10" style="border: 1px solid rgba(88, 17, 26, 0.1);">
      ...
      <div class="d-flex align-center justify-space-between mb-4">
        <div class="text-h5 font-weight-bold" style="color: #58111A; letter-spacing: -0.5px;">
          <v-icon class="mr-2" color="#58111A">mdi-account-plus-outline</v-icon>
          Add New Employee
        </div>
        <v-btn
          icon="mdi-close"
      ...
      <v-form ref="formRef" v-model="isFormValid" @submit.prevent="onSubmit">
        ...
        <div class="font-weight-bold mb-2">Temporary Password</div>
          <v-text-field
            v-model="password"
